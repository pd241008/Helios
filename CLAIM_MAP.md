...
| C4 | Chennai ensemble: base ensemble R²=0.790; literature-standard CatBoost (Base) is the best tuned model at R²=0.810; ensembling does not beat the already-strong single model | `experiment_manifests/C4-chennai-ensemble.json` | 8-model + 2-ensemble evaluation on identical split | `ml-python/helios_ml/ensemble.py --split-strategy marker` | paper Table 3; partial archive in `results/chennai_v2/ensemble_metrics_partial.json` |
| C5 | Bangalore: individual models are unstable under the thin seasonally-narrow test window (R²=0.27–0.36), but the four-model ensemble recovers R²=0.529 (tuned) / 0.513 (base) | `experiment_manifests/C5-bangalore-ensemble.json` | 8-model + 2-ensemble evaluation, rolling 12-month holdout | `ml-python/helios_ml/ensemble.py --split-strategy dynamic` | paper Table 5; see `results/README.md` |
